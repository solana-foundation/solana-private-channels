...
| `MintCloseAuthority` (Token-2022) | Once supply reaches zero the mint can be closed and re-created at the *same pubkey* with a different extension set (e.g. adding `PermanentDelegate` or `PausableConfig`). `AllowMint` pins the reviewed `decimals`, `token_program`, extension set and freeze-authority presence, so a recreate that changes any of them stops deposits with `MintProfileChanged` until an admin re-allows. What is *not* compared is an extension's contents (a fee raised, a hook program swapped or a permanent delegate rotated leaves the profile identical) and the metadata, group and group-member bits, which an issuer can add to a live mint. | Prefer rejecting mints that carry `MintCloseAuthority`; if accepted, treat the original review as void after any close/recreate. |
