...
- **Account-gated methods:** `getAccountInfo`, `getTokenAccountBalance`, `getSignaturesForAddress`. JWT required. Operator role → pass through. User role → the gateway fetches the target account via an internal `getAccountInfo` and inspects bytes for the SPL Token owner field and looks the pubkey up in the auth service's Postgres `verified_wallets` table. A token account's delegate is accepted for `getAccountInfo` and `getTokenAccountBalance`, which read current state it can already spend, but not for `getSignaturesForAddress`: a delegation is a current spend authority and says nothing about who controlled the address when past transactions landed. Owning the account now is not enough for history either: the node records every landed `SetAuthority { AccountOwner }` in `token_account_owner_change`, and the gateway serves a User only the slots their own wallet held the address for, so an ownership handoff no longer carries the previous owner's signatures to the new one. Mismatch → 403, `-32002` ("account not owned by caller"). DB error → 500, `-32603`. Missing pubkey in params → 400, `-32602`. Read node unreachable, slow, or answering with an error → 503, `-32004`, which is retryable where 403 is not.
...
| `getSignaturesForAddress` **[auth]** | `limit`, `before`, `until` honoured. `minContextSlot` ignored. Default/max 1000 (matches Solana). Under auth, User role sees one uniform `err` per failed transaction instead of the real `TransactionError`; operators see the real one. A User's page is also scoped to the slots they owned the address for. The scope is applied in the query rather than to the page, so `limit` counts entries the caller may see and the `before`/`until` cursors behave exactly as they do unscoped. History stops at `getFirstAvailableBlock`: entries whose transactions were pruned by truncation are not returned, the way Solana ends history at its oldest retained slot. A `before` cursor naming a pruned signature returns an empty page, and an `until` cursor naming one returns every retained entry. |
