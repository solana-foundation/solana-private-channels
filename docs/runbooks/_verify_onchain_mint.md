...
- No match, `processed_at` is older than the time of the first available
  block (`getBlockTime` of the slot `getFirstAvailableBlock` returns, in Unix
  seconds, e.g. `date -u -d @<block-time>`), and the history is empty or
  starts at that block: output `AMBIGUOUS`. Truncation
