...
| `error_message` contains | Cause |
|---|---|
| `is not in the allow-listed mints table` | The deposit's `mint` has no row in `mints`. The operator refused to issue private channel tokens because no indexed `AllowMint` event authorizes this mint. Row data is fine; no on-chain mint attempted. See **Path F**. |
