...
- Records the mint's `decimals` and `token_program` as the reviewed profile. `Deposit` rejects the mint with `MintProfileChanged` if either changes, which an admin clears by blocking and re-allowing (a re-allow also re-opens both gates). A decimals change also needs the channel mint re-created, since re-allow leaves it on the old decimals
